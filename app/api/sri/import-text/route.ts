...
export async function POST(req: Request) {
  try {
    ...
    const fileName = (body?.fileName || "SRI.txt") as string;
    const contentBase64 = body?.contentBase64 as string;

    if (!contentBase64) {
      return NextResponse.json({ ok: false, error: "Falta contentBase64." }, { status: 400 });
    }

    const txtContent = Buffer.from(contentBase64, "base64").toString("utf-8");
    ...
    return NextResponse.json({
      ok: true,
      summary: { file: fileName, sha256: txtHash, totalRows: data.length, inserted, duplicates },
    });
  } catch (e: any) {
